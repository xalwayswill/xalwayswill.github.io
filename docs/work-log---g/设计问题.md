...
47. SPI Master 存在一个eco问题，里面的rx_fifo使用的dw FIFO，只能选择同步或者异步复位，模块是有同步和异步复位同时输入的，内部将这两个信号通过一个DFF转成了同步复位给到FIFO，但是异步复位时候是没有clk的，所以导致时钟出现的第一个cycle FIFO是没有复位的，rx_data_valid是通过~rx_fifo_empty来给出去的，就会导致上电之后empty信号不能立即复位，必须等待时钟出来后一拍才能复位（以后还是少用这种异步同步复位合并的逻辑吧，找支持异步同步复位的）
48. ECO比较简单，插入一个AND2 将fifo_rst_n 和 fifo_empty_n与起来，通过connect_cell/disconnect_net/connect_pin/connect_net等命令完成网表修改，需要将dc/dft/pr网表全部修改，dft会在dc基础上增加dft逻辑，pr网表会在dft上插入很多buffer，需要注意修改不能影响dft功能，dff Q 到 SI的路径及值不能改变

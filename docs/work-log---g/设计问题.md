...
39. SRAM 选择2p还是sp
  ...
  5. SRAM 的LEF不一定是wordline 水平，bitline垂直，用的SNPS的umc22的height就是通过mux控制，也就是说bit垂直排列，bank水平划分
40. SRAM 一些功能
  1. LS/DS/SD，根据文档描述，lS/DS/SD 分别通过 enable biasing, partial periphery shutdown, full periphery shutdown with data retention, and acomplete shutdown without data retention.的方式来节约静态功耗，唤醒时间各不相同，另外SNPS还支持dual-rail供电，将外设与arry供电独立，在关闭外部供电同时减低array电压等操作
  2. sram的一些调速接口，RM（read margin） RA/WA read/write assistance等都是为了给bit两端的word line提供更多的充放电时间，来保证数据的稳定性，但是会降低速度
  3. sram的读写功耗和bank mux 有直接关系，bank大的话sram的速度更快同时功耗控制更细（类似大的SRAM拆分成小的），没有参与读写的区域能够降低功耗，但是不绝对

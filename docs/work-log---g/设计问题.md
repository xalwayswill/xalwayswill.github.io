...
28. SPI Master SDC -- 当前设计的spi master的SCK是lf_clk二分频产生，MOSI同样是lf_clk驱动，虽然设置的output delay 是基于SCK的，SCK上升沿去采样数据MOSI，但是因为MOSI是lf_clk驱动且只在SCK的下降沿更新，所以会导致MOSI在SCK采样时的hold 检查过于严格（setup正常）
```
  _   _   _   _   _   _
_| |_| |_| |_| |_| |_| |_
 A   B   C   D   E   F

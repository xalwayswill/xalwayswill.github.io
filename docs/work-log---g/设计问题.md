...
```
47. DMUX模块的最前级一定不要忘记用输入valid（pulse）对输入数据Latch下，万一输入数据不是DFF输出或者hold不满足就可能出问题，并且用输入的pulse去latch数据还能检查valid和数据没有对齐的问题（自己为何脑残了下写出来个这代码），同时也要注意不管common ip 多烂，能用还是得用，不然出问题就是自己的
